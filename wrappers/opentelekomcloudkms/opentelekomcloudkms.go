...
const (
	...
	EnvOpenTelekomCloudKmsKeyId        = "VAULT_OPENTELEKOMCLOUDKMS_KEY_ID"
)
...
// NewWrapper creates a new OpenTelekomCloudKmsWrapper
...
// SetConfig sets the fields on the OpenTelekomCloudKmsWrapper object based on
...
// Encrypt is used to encrypt the master key using the the Open Telekom Cloud CMK.
...
func buildKMSClient(opts *options) (kmsClient, error) {
	...
	region, err := getConfig("region", os.Getenv("OPENTELEKOMCLOUD_REGION"), opts.withRegion)
	if err != nil {
		return nil, err
	}

	// Check and set project.
	project, err := getConfig("project", os.Getenv("OPENTELEKOMCLOUD_PROJECT"), opts.withProject)
	if err != nil {
		return nil, err
	}

	// Check and set access key.
	accessKey, err := getConfig("access_key", os.Getenv("OPENTELEKOMCLOUD_ACCESS_KEY"), opts.withAccessKey)
	if err != nil {
		return nil, err
	}

	// Check and set project.
	secretKey, err := getConfig("secret_key", os.Getenv("OPENTELEKOMCLOUD_SECRET_KEY"), opts.withSecretKey)
	if err != nil {
		return nil, err
	}
...
func buildServiceClient(option golangsdk.AKSKAuthOptions) (*golangsdk.ServiceClient, error) {
	client, err := openstack.NewClient(option.IdentityEndpoint)
	if err != nil {
		return nil, err
	}
	...
	err = openstack.Authenticate(client, option)
	if err != nil {
		return nil, err
	}

	return openstack.NewKMSV1(client, golangsdk.EndpointOpts{
...
func (c *kmsClientImpl) encrypt(keyID, plainText string) (encryptResponse, error) {
	// TODO: waiting for https://github.com/opentelekomcloud/gophertelekomcloud/issues/524
